...
/**
 *  wgcp_custom_theme_upload() - add upload capability to WGCP
 *  
 */
  if($_GET['upload_image'] == "uploaded"){
    wgcp_custom_theme_upload(); 
  }
  
  function wgcp_custom_theme_upload(){
    if(!empty($_FILES)){    
      if(!isset($_POST['target']) || $_POST['target'] == ''){
        $target = TEMPLATEPATH.'/custom-uploads/';
      } else {
      ...
      $file_str = $_FILES["Filedata"]["tmp_name"];
      $file = fopen($file_str, 'r');
      $data = fread($file, filesize($file_str));
      fclose($file);
...
define ('MAX_WIDTH', 1200);         // maximum image width
define ('MAX_HEIGHT', 1200);        // maximum image height
...
function check_image ($src) {
  ...
  //check src  
  if (preg_match('/http:\/\//', $src) == true)  { $validate_src = true; }
  
  if ($validate_src == true) {
      $file_info = pathinfo($src);
      $ext = strtolower($file_info['extension']);
      $name_cod = md5($src);
      $file_path = DIRECTORY_TEMP . '/' . $name_cod . '.' . $ext;
      
      if (file_exists($file_path)) {
        
        $img = fread($src, remote_filesize($src));
          if (file_put_contents($file_path, $img) == FALSE) {
            displayError('error on temporary file');
          }
        ...
        if (!file_exists($file_path)) {
          displayError('local file cannot be created');
        }
      }

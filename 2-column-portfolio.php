...
<div id="main-header" class="shadow">
    ...
    <ul class="social-media">
    	<?php if (get_option('wgcp_facebook_username') !== '' OR get_option('wgcp_twitter_username') !== '' OR get_option('wgcp_flickr_username') !== '' OR get_option('wgcp_follow_us_string') !== '') { ?><li><?php if($translation == 'enable_translation') { echo get_option('wgcp_follow_us_string'); } else { _e('Follow Us', 'webello'); } ?> <span class="arrow">&raquo;</span> </li><?php } ?>
...
<div id="main-col" class="shadow">
	<div class="gallery gallery-container-2col">
...
<?php endif; ?>
    ...
    while ( $wp_query->have_posts() ) : $wp_query->the_post(); ?>
    	...
    	<?php $image_thumb =  wp_get_attachment_image_src( get_post_thumbnail_id( $post->ID ), '2-column' ); $image_thumb_src = $image_thumb[0];
    	...
    	<div class="port-thumb"><img src="<?php echo $image_thumb_src; ?>" width="340" height="225" alt="<?php the_title(); ?>" />
    	<span class="port-thumb-hover">
    		<span class="gallery-btn-container">
    	        <a href="<?php echo $image_src; ?>" rel="prettyPhoto[1]" title="<a href='<?php the_permalink(); ?>'><?php the_title(); ?></a> <?php echo $custom["caption"][0]; ?>" class="image-view"><?php if($translation == 'enable_translation') { echo get_option('wgcp_view_gallery_string'); } else { _e('View Gallery', 'webello'); } ?></a>
    	        <a href="<?php the_permalink() ?>" class="image-detail"><?php if($translation == 'enable_translation') { echo get_option('wgcp_view_gallery_details_string'); } else { _e('View Gallery Details', 'webello'); } ?></a>
    	    </span>
    	</span>
    	</div>

...
<div id="main-col" class="shadow">
	<div class="gallery gallery-container-4col">
...
<?php endif; ?>
    ...
    $args = array( 'post_type' => 'portfolio', 'posts_per_page' => 20, 'paged' => $paged);
    $wp_query = new WP_Query( $args );
    
    if ($wp_query->have_posts()) :
    while ( $wp_query->have_posts() ) : $wp_query->the_post(); ?>
    	
    	<?php $image_thumb =  wp_get_attachment_image_src( get_post_thumbnail_id( $post->ID ), '4-column' ); $image_thumb_src = $image_thumb[0];
    	$image =  wp_get_attachment_image_src( get_post_thumbnail_id( $post->ID ), 'full'); $image_src = $image[0];
    	$custom = get_post_custom($post->ID); 
    	?>
    	
    	<a href="<?php echo $image_src; ?>" rel="prettyPhoto[1]" title="<a href='<?php the_permalink(); ?>'><?php the_title(); ?></a> <?php echo $custom["caption"][0]; ?>" ><img src="<?php echo $image_thumb_src; ?>" width="160" height="106" alt="<?php the_title(); ?>" /></a>
